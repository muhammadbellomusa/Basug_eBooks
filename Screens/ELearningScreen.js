...
const ELearningScreen = ({ route, navigation }) => {
  ...
  async function schedulePushNotification() {
    await Notifications.scheduleNotificationAsync({
      content: {
        title: `A new Vodeo Course Added`,
        body: `New Video course added at ${selectedCourse} level ${selectedLevel}`,
        data: { data: 'goes here' },
      },
      trigger: { seconds: 2 },
    });
  ...
  const uploadVideo = async () => {
    ...
    try {
      ...
      const storageRef = firebase.storage().ref(`videos/${selectedCourse}/${videoName}`);
      ...
      await firebase.firestore().collection('videos').add({
        ...
        level: selectedLevel,
        // You can also include additional data here if needed
      });
      ...
      setUploading(false);
      Alert.alert(`Content uploaded to ${selectedCourse} level ${selectedLevel},${videoName} uploaded successfully`)
    }
  };

  // Function to retrieve videos based on the selected course and level
  useEffect(() => {
  ...
  return (
    <View style={{ flex: 1, justifyContent: 'space-between' }}>
      <Text style={{ textAlign: 'center', backgroundColor: 'yellow',
       height: 27, fontSize: 20, color: 'navy' }}>{selectedCourse}  eLearning Level{selectedLevel}</Text>
      ...
      {videoUri && (
        <View style={{ alignItems: 'center', marginVertical: 10 }}>
          {uploading ? (
            ...
            <Button title="Upload Video" onPress={uploadsVideo} />
          )}
        </View>
      )}
